...
추가 검색어 `id:<number>`, `status:<`[TaskStatus](../../source/modules/download.ts)`>` 를 지원합니다.<br>
또한 `AND`, `INCLUDE`, `EXCLUDE` 3가지의 접두어를 지원합니다.<br>
접두어는 낮말 혹은 묶음의 앞에 추가해 사용 가능하며, 기본 접두어는 `AND` 입니다.<br>

#### **AND**

**해당 접두어가 붙은 검색어의 결과**와 이전 결과 간의 **공통**된 결과를 연산합니다.<br>
*이전 결과가 없을 경우 `INCLUDE`와 동일한 동작을 합니다*<br>
