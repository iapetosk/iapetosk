...
class Browser extends React.Component<BrowserProps> {
	...
	constructor(props: BrowserProps) {
		super(props);
		this.props = props;
		this.state = { query: { enable: true, options: { input: settings.query.input }, handler: { keydown: (value: string) => { this.mutate(BrowserEvent.QUERY, { ...this.state.query, options: { ...this.state.query.options, input: value } }); } } }, iterable: { blocks: [] }, paging: { enable: true, options: { size: 0, index: 0, metre: settings.paging.metre }, handler: { click: (value: number) => { this.mutate(BrowserEvent.PAGING, { ...this.state.paging, options: { ...this.state.paging.options, index: value } }); } } }, session: { history: [], version: 0 }, blocks: [[], 0] };

		window.addEventListener("keydown", (event) => {
			if (this.props.enable && !document.querySelectorAll("input:focus").length) {
				switch (event.key) {
					case "ArrowLeft": {
						this.mutate(BrowserEvent.PAGING, { ...this.state.paging, options: { ...this.state.paging.options, index: utility.clamp(this.state.paging.options.index - 1, 0, this.state.paging.options.size - 1) } });
						break;
					}
					case "ArrowRight": {
						this.mutate(BrowserEvent.PAGING, { ...this.state.paging, options: { ...this.state.paging.options, index: utility.clamp(this.state.paging.options.index + 1, 0, this.state.paging.options.size - 1) } });
	...
	public mutate(section: BrowserEvent, $new: any) {
		switch (section) {
			case BrowserEvent.SESSION: {
				this.setState({ ...this.state, session: $new }, () => {
					this.mutate(BrowserEvent.BLOCKS, [[], 0]);
						...
						for (let index = 0; index < blocks.length; index++) {
							read.block(array[index + (singular ? 0 : $new.history[$new.version][1] * settings.hitomi.per_page)]).then((block) => {
								...
								if (Object.keys(blocks).length === blocks.length) {
									this.mutate(BrowserEvent.BLOCKS, [blocks, size]);
								}
							});
						}
					});
			...
			case BrowserEvent.QUERY: {
				this.setState({ ...this.state, query: $new }, () => {
					this.mutate(BrowserEvent.PAGING, { ...this.state.paging, options: { ...this.state.paging.options, index: 0 } });
				});
				suggest.up();
				settings.query = { ...settings.query, input: $new.options.input };
				break;
			}
			case BrowserEvent.ITERABLE: {
				this.setState({ ...this.state, iterable: $new });
				break;
			}
			case BrowserEvent.PAGING: {
				this.setState({ ...this.state, paging: $new }, () => {
					this.mutate(BrowserEvent.SESSION, { history: [...this.state.session.history, [this.state.query.options.input, $new.options.index]], version: this.state.session.version + 1 });
				});
				break;
			}
		}
	...
	public componentDidMount() {
		this.mutate(BrowserEvent.SESSION, { history: [[settings.query.input, 0]], version: 0 });
	}
	public render() {
		return (
