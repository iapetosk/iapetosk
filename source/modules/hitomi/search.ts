...
class Search {
	...
	public get(filter: SearchQuery, page: { size: number, index: number; }): Promise<SearchResponse> {
		return this.unknown_0(filter, page);
	}
	public unknown_0(filter: SearchQuery, page: { size: number, index: number; }): Promise<SearchResponse> {
		...
		return new Promise<SearchResponse>((resolve, rejects) => {
			for (const type of Object.keys(filter)) {
				switch (type) {
					case "language": {
						...
						for (let index: number = 0; index < filter[type as SearchType].length; index++) {
			...
			if (URLs[SearchAction.POSITIVE].length === 0) {
				URLs[SearchAction.POSITIVE].unshift(Search.index_all);
			}

			const SINGULAR: boolean = URLs[SearchAction.POSITIVE].length === 1 && URLs[SearchAction.NEGATIVE].length === 0 && URLs[SearchAction.POSITIVE][0] === Search.index_all;

			function $(action: SearchAction, array: number[]): void {
			...
			for (let index: number = 0; index < URLs[SearchAction.POSITIVE].length + URLs[SearchAction.NEGATIVE].length; index++) {
				const shortcut: {
					action: SearchAction,
					url: string;
				} = {
				...
				if (SINGULAR || !Search.collection[shortcut.url]) {
					...
					$(shortcut.action, Search.collection[shortcut.url]);
				}
			}
		});
	}
	private unknown_1(response: RequestResponse): number[] {
